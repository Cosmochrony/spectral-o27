...
\date{2026\\[2pt]\small Version 1.15.4}
...
\begin{document}
  ...
  \begin{abstract}
    ...
    This upgrades the result of O23~\cite{Beau2026a27}---which proves $\dim_{\mathbb{R}}
    (\mathrm{Im}\,\mathbb{H}) = 3$ for the traceless sector of a supplied spinor carrier
    (O23 Theorem~3.1)---from a dimension count to a categorical universality statement:
    $\mathfrak{su}(2)$ is the unique minimal admissible target through which every $\Phi_{q,\rho}$
    must factor.
    The universality statement is conditional on that supplied carrier; the carrier selection
    and the observable identification remain open bridges of O23.
  ...
  \subsection{The answer: a rigidity theorem}
    ...
    $H_{\mathrm{eff}} \simeq \mathfrak{su}(2)$ (O23~\cite{Beau2026a27}, conditional on the
    supplied spinor carrier) is forced by the interplay of
    ...
    \begin{table}[h]
      ...
      \begin{tabular}{lcccl}
        ...
        \hline
      \end{tabular}
      \caption{Compatibility of candidate target structures with constraints~(a)--(c). The
      ``Arbitrary Hilbert space'' row is established for $V_\rho$ of quaternionic representation
      type; see Remark~\ref{rem:hilbert-scope}.}
  ...
  The parity involution $c \leftrightarrow q-c$, the candidate fibre structure of the pair
  sector (a structurally motivated hypothesis; O18~\cite{Beau2026a22} establishes
  Born--Infeld parity as a covariance of the response family and states the fibre identification as an
  open problem), acts on $V_q$ by complex conjugation:
  $V_{q-c} \simeq \overline{V_c}$ (the conjugation identity of O17, recalled in O18
  Theorem~3.1).
  ...
  \]
  with $\dim_{\mathbb{R}} H_{\mathrm{eff}} = 3$ by O23 Theorem~3.1~\cite{Beau2026a27},
  conditionally on the supplied spinor carrier $V_\rho \cong \mathbb{C}^2$ assumed there
  (the carrier selection and the identification of the observable sector with
  $\mathfrak{su}(V_\rho)$ are open bridges of O23), together
  ...
  \begin{proof}
    ...
    \mathrm{Ntrl}$.
    By O23 Theorem~3.1~\cite{Beau2026a27}, under the supplied spinor carrier hypothesis,
    $H_{\mathrm{eff}} \cong \mathfrak{su}(2) \cong \mathrm{Im}\,\mathbb{H}$ is quaternionic
    and its real rank is exactly~3.
  ...
  \begin{proposition}[Generic complex Hilbert spaces are not canonical, for quaternionic-type targets]
    \label{prop:hilbert}
    Let $V_\rho$ be of quaternionic representation type (half-integer spin under $\mathrm{SU}(2)$).
  ...
  \begin{proof}
    ...
    $V_\rho$ as an $\mathrm{SU}(2)$-subrepresentation.
    For $V_\rho$ of quaternionic type, as assumed here, this containment is equivalent to
    $\mathcal{H}$ carrying a compatible quaternionic structure, since the commutant of a
    quaternionic-type representation is itself quaternionic.
    This equivalence is specific to the quaternionic case (see Remark~\ref{rem:hilbert-scope} below):
    for $V_\rho$ of real type (integer spin), whose commutant is $\mathbb{R}$, the same containment
    condition would constrain only a real structure on $\mathcal{H}$, not a quaternionic one.
  \end{proof}
  ...
  \begin{remark}[Scope of Proposition~\ref{prop:hilbert}]
    \label{rem:hilbert-scope}
    Proposition~\ref{prop:hilbert} is stated and proved for $V_\rho$ of quaternionic representation
    type.
    The commutant of an irreducible real representation of a compact group is $\mathbb{R}$,
    $\mathbb{C}$, or $\mathbb{H}$ according to its (real, complex, or quaternionic) representation
    type; for $\mathrm{SU}(2)$, half-integer-spin representations are of quaternionic type and
    integer-spin representations are of real type.
    Only in the quaternionic case does containing $V_\rho$ as an $\mathrm{SU}(2)$-subrepresentation
    force the ambient space to carry a compatible quaternionic structure.
    For $V_\rho$ of integer spin, this proposition's argument does not exclude a generic complex
    Hilbert space as a candidate target, and the exclusion of non-quaternionic candidates for that
    case is not established here.
    This does not affect Theorem~\ref{thm:rigidity}: the rigidity of $H_{\mathrm{eff}} \simeq
    \mathfrak{su}(2)$ itself is established independently, via the admissible-quotient factorisation
    of Lemma~\ref{lem:quotient}, the conditional dimension count $\dim_{\mathbb{R}}
    H_{\mathrm{eff}} = 3$ of O23
    Theorem~3.1~\cite{Beau2026a27}, and the classical uniqueness of $\mathbb{H}$ (Frobenius
    theorem), and does not rely on Proposition~\ref{prop:hilbert}.
  \end{remark}
  ...
  \begin{proof}
    ...
    Hence the group of such automorphisms is $\mathrm{SU}(2)$, acting on
    $H_{\mathrm{eff}} \simeq \mathfrak{su}(2)$ by the adjoint action, and
    $H_{\mathrm{eff}} \simeq \mathfrak{su}(2)$ is forced.
...
O23~\cite{Beau2026a27} proves $\dim_{\mathbb{R}} H_{\mathrm{eff}} = 3$ conditionally on the
supplied spinor carrier (O23 Theorem~3.1).
Theorem~\ref{thm:rigidity} upgrades this from a dimension statement to a \emph{categorical
universality} statement, under the same supplied carrier: $\mathfrak{su}(2)$ is the unique
minimal admissible target through
...
Dimension~1 ($\mathbb{R}$) is excluded by the non-commutativity imposed by~(c) and by the
non-abelian support lemma of O23 (Lemma~2.1 there, carried by irreducibility of the supplied
carrier, not by neutrality).
...
excluded because $\mathrm{dim}_{\mathbb{R}} H_{\mathrm{eff}} = 3$ (O23 Theorem~3.1, under
the supplied carrier).
...
conditional dimension bound of O23; it does not depend on the Heisenberg group structure or on
...
\[
  ...
  \Rightarrow \text{quadratic form} \Rightarrow \mathfrak{su}(2).
\]
The arrows are of unequal type.
The first is established in paper~L~\cite{Beau2026l}.
The second is partly open: O18~\cite{Beau2026a22} proves Born--Infeld parity as a
response-family covariance, but the identification of the parity orbit with a fibre of $\Pi$
--- hence the pair structure --- is a structurally motivated hypothesis (O18 Problem~2.8).
The third is established in O26~\cite{Beau2026a30}; the last arrow is
Theorem~\ref{thm:rigidity}, conditional on the supplied carrier of O23.
  \end{remark}
