...
\begin{document}
  ...
  \begin{abstract}
    ...
    must factor.
    Combined with the chain $c_{\mathrm{BI}} \to \delta_{\mathrm{pair}} \to \beta^*$ of
    O24~\cite{Beau2026a28}, this gives $\beta^*$ a representation-theoretic meaning as the
    effective scaling exponent of norm growth in the minimal admissible non-abelian sector.
  ...
  \subsection{Relation to the O-series chain}

    The transfer $c_{\mathrm{BI}} \to \delta_{\mathrm{pair}} \to \beta^*$ was proved in
    O24~\cite{Beau2026a28}, unconditionally with respect to the fibre structure of $\Pi$
    (its standing conditions being the O4--O6 growth-equation closure and coupling
    hypotheses, the Weil realisation, and the power-law regime).
    The present result adds a representation-theoretic layer to that chain:
    \[
    ...
    The exponent $\beta^*$ thereby acquires a direct representation-theoretic meaning:
    it is the effective scaling exponent of norm growth along a trajectory in the minimal admissible
  ...
  \subsection{Scope}
    ...
    It does not revisit the transfer chain, proved in O24~\cite{Beau2026a28}.
  ...
  \subsection{Organisation}
    ...
    Section~\ref{sec:chain} derives the representation-theoretic interpretation of $\beta^*$.
...
properties specific to any prime $q$.
  ...
  \section{Representation-Theoretic Interpretation of $\beta^*$}
  \label{sec:chain}
  ...
  \]
  realising $\delta_{\mathrm{pair}}$ as the growth exponent of a Hilbert--Schmidt norm along a
  trajectory $(\widetilde{M}^{(n)})_{n \geq 0}$ in $\mathrm{End}(V_\rho)$.

  \begin{corollary}[Representation-theoretic meaning of $\beta^*$]
    \label{cor:beta}
    Under the transfer $c_{\mathrm{BI}} \to \delta_{\mathrm{pair}} \to \beta^*$ of
    O24~\cite{Beau2026a28} (unconditional with respect to the fibre structure of $\Pi$),
    the exponent $\beta^*$ is the effective scaling exponent of norm growth
    in the minimal admissible non-abelian sector $\mathfrak{su}(2)$.
    The phenomenological window $\beta^* \in (0.09, 0.13)$ is a constraint on admissible
    trajectories in the representation space $\mathrm{End}(V_\rho)$, not a purely numerical fit.
  \end{corollary}

  \begin{remark}[Completeness of the chain]
The full implication chain is now:
\[
  \text{emergence} \;\Rightarrow\; \text{non-injectivity}~\text{\cite{Beau2026l}}
  \;\Rightarrow\; \text{pair structure}~\text{\cite{Beau2026a22}}
  \;\Rightarrow\; \text{quadratic form}~\text{\cite{Beau2026a30}}
  \;\Rightarrow\; \mathfrak{su}(2)~\text{(Theorem~\ref{thm:rigidity})}.
\]
